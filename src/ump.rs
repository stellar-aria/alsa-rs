...
use core::ffi::CStr;
use core::ptr;
use libc::{c_int, c_short, c_uint, pollfd, size_t, EINVAL};
...
impl Ump {
    /// Opens a UMP device (convenience wrapper for `&str` names)
    pub fn new(name: &str, dir: UmpDirection, nonblock: bool) -> Result<Self> {
        let c = CString::new(name).map_err(|_| Error::new("CString::new", EINVAL))?;
        Self::open(&c, dir, nonblock)
    }
    ...
    /// * `dir` - Direction (input or output; bidirectional is not supported here)
    /// * `nonblock` - Whether to open in non-blocking mode
    pub fn open(name: &CStr, dir: UmpDirection, nonblock: bool) -> Result<Self> {
        // FIXME: alsa::SND_UMP_NONBLOCK does not exist in alsa-sys
        const SND_UMP_NONBLOCK: c_int = 2;
        let mode = if nonblock { SND_UMP_NONBLOCK } else { 0 };
        ...
        let (inp, outp) = match dir {
            ...
            UmpDirection::Output => (
                ptr::null_mut(),
                &mut output_ptr as *mut *mut alsa::snd_ump_t,
            ),
            UmpDirection::Bidirection => {
                return Err(Error::new("snd_ump_open", EINVAL));
            }
        };
    ...
    ///
    /// # Safety
    /// The returned pointer is unowned and has no lifetime tie to `self`. The caller must
    /// not use it after `self` is dropped.
    pub unsafe fn rawmidi(&self) -> *mut alsa::snd_rawmidi_t {
        alsa::snd_ump_rawmidi(self.0)
    }
...
impl UmpEndpointInfo {
    ...
    pub fn get_sw_revision(&self) -> [u8; 4] {
        ...
        unsafe {
            ...
            result.copy_from_slice(core::slice::from_raw_parts(ptr, 4));
        }
        result
    }
...
impl UmpBlockInfo {
    ...
    pub fn get_direction(&self) -> Option<UmpDirection> {
        let dir = unsafe { alsa::snd_ump_block_info_get_direction(self.0) };
        match dir {
            alsa::SND_UMP_DIR_INPUT => Some(UmpDirection::Input),
            alsa::SND_UMP_DIR_OUTPUT => Some(UmpDirection::Output),
            alsa::SND_UMP_DIR_BIDIRECTION => Some(UmpDirection::Bidirection),
            _ => None,
        }
    }
...
fn test_ump_block_info() {
    ...
    assert_eq!(info.get_direction(), Some(UmpDirection::Bidirection));
}

#[test]
fn test_ump_endpoint_info_full() {
    let info = UmpEndpointInfo::empty().unwrap();

    info.set_flags(0x5);
    assert_eq!(info.get_flags(), 0x5);

    info.set_protocol_caps(0x3);
    assert_eq!(info.get_protocol_caps(), 0x3);

    info.set_protocol(0x1);
    assert_eq!(info.get_protocol(), 0x1);

    // sw_revision on a cleared struct must not crash
    let _ = info.get_sw_revision();

    // name and product_id are accessible after clear
    assert!(info.get_name().is_ok());
    assert!(info.get_product_id().is_ok());

    let name = core::ffi::CStr::from_bytes_with_nul(b"TestEndpoint\0").unwrap();
    info.set_name(name);
    assert_eq!(info.get_name().unwrap(), "TestEndpoint");

    let pid = core::ffi::CStr::from_bytes_with_nul(b"ProdID\0").unwrap();
    info.set_product_id(pid);
    assert_eq!(info.get_product_id().unwrap(), "ProdID");
}

#[test]
fn test_ump_block_info_full() {
    let info = UmpBlockInfo::empty().unwrap();

    // A zeroed direction field (0) is not a valid UmpDirection — must return None
    assert_eq!(info.get_direction(), None);

    info.set_active(1);
    assert_eq!(info.get_active(), 1);

    info.set_first_group(2);
    assert_eq!(info.get_first_group(), 2);

    info.set_num_groups(4);
    assert_eq!(info.get_num_groups(), 4);

    info.set_ui_hint(UmpBlockUIHint::Both);
    assert_eq!(info.get_ui_hint(), UmpBlockUIHint::Both);

    let name = core::ffi::CStr::from_bytes_with_nul(b"Block0\0").unwrap();
    info.set_name(name);
    assert_eq!(info.get_name().unwrap(), "Block0");

    // direction round-trip for each valid variant
    for dir in [
        UmpDirection::Input,
        UmpDirection::Output,
        UmpDirection::Bidirection,
        ...
        assert_eq!(info.get_direction(), Some(dir));
    }
}

#[test]
fn test_ump_open_bidirection_returns_error() {
    let result = Ump::open(
        core::ffi::CStr::from_bytes_with_nul(b"hw:0\0").unwrap(),
        UmpDirection::Bidirection,
        false,
    );
    assert!(
        result.is_err(),
        "Ump::open with Bidirection must return Err"
    );
}
